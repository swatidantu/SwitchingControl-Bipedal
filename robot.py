...
from walking_generator import PreviewControl as wt

class Robot:
    def __init__(self, robotPATH, startPosition, startOrientation, maxForce, controlMode=pb.POSITION_CONTROL, planePATH="stadium.sdf"):
        physicsClient = pb.connect(pb.GUI)
        pb.setAdditionalSearchPath(pybullet_data.getDataPath())
        pb.setGravity(0,0,-9.8)
    #    self._planeId = pb.loadURDF(planePATH)
        self._planeId = pb.loadSDF(planePATH)
...
class Bipedal(Robot):
    ...
    def __init__(self, startPosition=[0,0,0.55], startOrientation=[0,0,0], CoMposition_b=np.array([0.,0.,-0.02]),maxForce=9.0, controlMode=pb.POSITION_CONTROL, robotPATH="urdf/bipedal.urdf", planePATH="stadium.sdf"):
        ...
        self._A1 = 0.18
        self._A2 = 0.18
        self.RA = np.array([0,-0.130,0.120]) - CoMposition_b #from CoM to hipyaw joint
        self.LA = np.array([0,0.130,0.120]) - CoMposition_b
        self.LEG_DOF = 6
        self._jointIdListRA = [0,1,2,3,4]
        self._jointIdListLA = [5,6,7,8]
        self._jointIdListR = [10,11,12,13,14,15]
        self._jointIdListL = [16,17,18,19,20,21]
    ...
    def setRightLegJointPositions(self, targetJointPositions,targetJointTorques):
    ...
    #    pb.setJointMotorControlArray(self._robotId, jointIndices=self._jointIdListR, controlMode=pb.TORQUE_CONTROL, forces=targetJointTorques,self._maxForceListForLeg)
    def setLeftLegJointPositions(self, targetJointPositions,targetJointTorques):
    ...
    #    pb.setJointMotorControlArray(self._robotId, jointIndices=self._jointIdListL, controlMode=pb.TORQUE_CONTROL, forces=targetJointTorques)
    ...
    def getJointVelocities(self, leg):
        if np.sum(leg == self.R) == len(leg):
            jointStates = pb.getJointStates(self._robotId, jointIndices=self._jointIdListR)
            jointVelocities = [jointStates[i][1] for i in range(len(jointStates))]
    ...
    def getJointTorques(self, leg):
        if np.sum(leg == self.R) == len(leg):
            jointStates = pb.getJointStates(self._robotId, jointIndices=self._jointIdListR)
            jointTorques = [jointStates[i][2] for i in range(len(jointStates))]
    ...
    def adaptiveRobust1(self,qref,q,V,leg):

        tau = wt.adaptiveRobustSolve1(self,leg=leg,p_ref=qref,p=q,v=V,positionGain=900,velocityGain=60,omega = 1,E_bar = 0.1,Theta_dot = [0.0,0.0,0.0] , Gamma_dot = [0.0,0.0,0.0])
        #print((tau))
        return tau

    def adaptiveRobust2(self,qref,q,V,leg):

        tau = wt.adaptiveRobustSolve2(self,leg=leg,p_ref=qref,p=q,v=V,positionGain=900,velocityGain=60,omega = 1,E_bar = 0.1 ,Theta_dot = [0.0,0.0,0.0] , Gamma_dot = [0.0,0.0,0.0])
    ...
    def positionInitialize(self, startCOMheight=0.45, initialLegRPY=[0,0,0], initializeTime=1.0, initialJointPosRL=[0.0,0.0,-0.44,0.88,-0.44,0.0]):
        ...
        for i in initializeStep: 
            self.setLeftLegJointPositions(initialJointPosRL,self._maxForceListForLeg)
            self.setRightLegJointPositions(initialJointPosRL,self._maxForceListForLeg)
        ...
        for i in initializeStep:
            ...
            self.setRightLegJointPositions(PosR,self._maxForceListForLeg)
            self.setLeftLegJointPositions(PosL,self._maxForceListForLeg)
